...
st.markdown('<div class="subtitle-style">Upload an image of your ingredients and get delicious recipe ideas instantly!</div>', unsafe_allow_html=True)
...
if uploaded_file:
    ...
    if detected:
        st.write("**Detected ingredients:**", ', '.join(
            f'<span class="ingredient-badge">{d["name"].capitalize()} ({d["confidence"]*100:.1f}%)</span>' for d in detected
        ), unsafe_allow_html=True)
    else:
    ...
    if recipes:
        ...
        st.markdown('<div style="display:flex;align-items:center;font-size:1.5rem;color:#22223b;font-weight:600;margin-top:1.5em;margin-bottom:0.5em;"><span style="font-size:2rem;margin-right:0.5em;">🧺</span>Suggested Recipes:</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div style="margin-top:2em; color:#4f8bf9; font-size:1.1rem;">👈 Upload an image to get started!</div>', unsafe_allow_html=True)
